...
class GamificationService {
  ...
  List<String> checkBadges({
    ...
    if (newEntry.barcode.length > 3) {
      final barcodeCount =
          userPastEntries.where((e) => e.barcode.length > 3).length + 1;
    ...
    if (newEntry.hasReceipt) {
      final receiptCount =
          userPastEntries.where((e) => e.hasReceipt).length +
          (newEntry.hasReceipt ? 1 : 0);
    ...
    try {
      final product = allProducts.firstWhere(
        ...
        orElse: () =>
            Product(barcode: '', name: '', brand: '', category: '', source: ''),
      );
      ...
      if (product.barcode.isNotEmpty) {
        ...
        if (cat.contains('meyve') ||
            cat.contains('sebze') ||
            cat.contains('domates')) {
          unlock('manav');
        }
        if (cat.contains('ekmek') ||
            cat.contains('pasta') ||
            cat.contains('un')) {
          unlock('firinci');
        }
        if (cat.contains('çikolata') ||
            cat.contains('cips') ||
            cat.contains('bisküvi') ||
            cat.contains('abur')) {
          unlock('keyifci');
        }
        if (cat.contains('bebek') ||
            cat.contains('mama') ||
            cat.contains('bez')) {
          unlock('bebek_dostu');
        }
        if (cat.contains('deterjan') ||
            cat.contains('temiz') ||
            cat.contains('sabun')) {
          unlock('titiz');
        }
        if (cat.contains('şampuan') ||
            cat.contains('krem') ||
            cat.contains('diş') ||
            cat.contains('kozmetik')) {
          unlock('bakimli');
        }
        if (cat.contains('su ') ||
            cat.contains('gazoz') ||
            cat.contains('ola') ||
            cat.contains('içecek')) {
          unlock('icecek_uzmani');
        }
        if (cat.contains('kalem') ||
            cat.contains('defter') ||
            cat.contains('kağıt') ||
            cat.contains('ofis')) {
          unlock('kirtasiyeci');
        }
        if (cat.contains('giyim') ||
            cat.contains('pantolon') ||
            cat.contains('tişört') ||
            cat.contains('tekstil')) {
          unlock('modaci');
        }
        if (cat.contains('kedi') ||
            cat.contains('köpek') ||
            cat.contains('mama') ||
            cat.contains('pet')) {
          unlock('pati_dostu');
        ...
        if (cat.contains('pil') ||
          ...
          unlock('teknoloji_kurdu');
        }
        if (cat.contains('benzin') ||
            cat.contains('mazot') ||
            cat.contains('lpg') ||
            cat.contains('akaryakıt')) {
          unlock('sofor');
        }
      }
    } catch (_) {}
    ...
    // --- 6. Economy ---
    if (newEntry.price.toString().endsWith('.99') ||
        newEntry.price.toString().endsWith('.90')) {
    ...
    final historicalChains = userPastEntries.map((e) {
      try {
        return allBranches
            .firstWhere((b) => b.id == e.marketBranchId)
            .chainName;
      } catch (_) {
        return 'unknown';
      }
    ...
    // Add current one
    try {
      final currentChain = allBranches
          .firstWhere((b) => b.id == newEntry.marketBranchId)
          .chainName;
