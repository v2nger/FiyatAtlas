...
class _SubmitPriceLogScreenState extends ConsumerState<SubmitPriceLogScreen> {
  ...
  Future<String> _generateDeviceHash() async {
    ...
    if (Platform.isAndroid) {
      ...
      raw = '${ios.identifierForVendor}-${ios.model}-${ios.systemVersion}';
    } else {
      ...
      if (extracted != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                "No clear price detected, please enter manually.",
  ...
  Future<void> _submit() async {
    ...
    if (_priceController.text.isEmpty || _barcodeController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Barcode and Price are required",
            style: TextStyle(color: AppColors.textPrimary),
          ),
          backgroundColor: AppColors.error,
        ),
      ...
      if (newProduct == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                "Product creation cancelled. Cannot submit.",
                style: TextStyle(color: AppColors.textPrimary),
              ),
              backgroundColor: AppColors.warning,
            ),
    ...
    final log = PriceLog(
      ...
      marketName: _marketController.text.isEmpty
          ? "Default Market"
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Submit Price"), centerTitle: true),
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            ...
            style: _receiptImage != null
                ? BorderStyle.solid
                : BorderStyle
                      .none, // Dashed unsupported natively easily, sticking to clean border
          ),
